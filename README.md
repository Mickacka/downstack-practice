...
- **Rush (3 min)** gives you 3 minutes to solve as many puzzles as you can: solving one brings the next, and a miss moves straight on to a new puzzle. Your best score per mode is kept and shown on Your Progress.
...
- Each map is a bumpy mid-game stack with a 3-6 wide well (mostly 3-4 wide with one spin, 4-6 with two) and asks for **two spins in a row** (or one, in Options). Build the first setup in the well (and on the stack right beside it, filling its dips), then spin in; the lines it clears leave the base for the second setup, which you build and spin into the same way.
